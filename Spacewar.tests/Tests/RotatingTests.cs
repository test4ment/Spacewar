...
public class RotatingFeatures : Feature
{
    ...
    {
        rotating_object.Setup(obj => obj.angle).Returns(new int(ang1)).Verifiable();
        rotating_object.Setup(obj => obj.angle_velocity).Returns(new int(vel1)).Verifiable();
    }
    ...
    public void RotatingCantGetAng()
    {
        ...
        rotating_object.Setup(obj => obj.angle_velocity).Returns(new int (90));
    }

    [Given("Объект, у которого невозможно прочитать значение угловой скорости")]
    public void RotatingCantGetVel()
    {
        rotating_object.Setup(obj => obj.angle).Returns(new int (45));
